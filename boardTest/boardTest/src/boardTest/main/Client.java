...
public class Client {
	...
	private static void menu() {
		...
		try {
			...
			loginJSON.put("loginIf", loginok);
			loginJSON.put("loginId", loginId);
			int select_num = Integer.parseInt(br.readLine());

			switch (select_num) {
			case 1 -> memberMenu(loginJSON);
			case 2 -> productMenu(loginJSON);
			case 3 -> boardMenu(loginJSON);
			default -> {
	...
	// memberMenu
	public static void memberMenu(JSONObject loginJSON) {
		loginok = loginJSON.getInt("loginIf");
	...
	private static void loginMode() {
		...
		String member_menu = new StringBuilder().append("1.LOGOUT 2.MYINFO 3.SIGN_OUT 4.EXIT").toString();
	...
	// boardMenu
	public static void boardMenu(JSONObject loginJSON) {
		switch (loginok) {
		case 0 -> memberService.login();
		case 1 -> {
			...
			try {
				...
				switch (select_num) {
				case 1 -> boardService.boardEnroll(loginJSON);
				case 2 -> boardService.boardSelect(loginJSON);
				case 3 -> boardService.boardEdit(loginJSON);
				case 4 -> boardService.boardDelete(loginJSON);
				case 5 -> menu();
				}
	...
	// productMenu
	public static void productMenu(JSONObject loginJSON) {
		switch (loginok) {
		case 0 -> memberService.login();
		case 1 -> {
			...
			try {
				...
				switch (select_num) {
				case 1 -> productService.productEnroll(loginJSON);
				case 2 -> productService.productSelect(loginJSON);
				case 3 -> productService.productEdit(loginJSON);
				case 4 -> productService.productDelete(loginJSON);
				case 5 -> menu();
				}
